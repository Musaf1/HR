...
class CVsView extends StatefulWidget {
  ...
  @override
  State<CVsView> createState() => CVsViewState();
}

class CVsViewState extends State<CVsView> {
                ...
                child: Card(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () async => await Prosses().infoPerson(index, currentPersonList, context),
                    ),
                    IconButton(
                      color: Colors.teal[700],
                      icon: const Icon(Icons.check_outlined),
                      onPressed: () => Prosses().addPersonToInterview(index, currentPersonList, context, this),
                    ),
                    IconButton(
                      color: Colors.red[300],
                      icon: const Icon(Icons.delete_outline),
                      onPressed: () =>
                          Prosses().deletePerson(index, 'delete from cv  where id=?', currentPersonList, context, this),
                    ),
                    IconButton(
                      color: Colors.teal[300],
                      icon: const Icon(Icons.file_open_outlined),
                      onPressed: () => Prosses().openCV(currentPersonList[index].id),
                    ),
                    const SizedBox(
                      width: 8,
                    )
